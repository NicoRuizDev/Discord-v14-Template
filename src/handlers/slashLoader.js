...
async function slashLoader() {
  ...
  slashFolders.forEach((dir) => {
    ...
    for (const file of commands) {
      ...
      try {
        ...
        console.log(
          chalk.white(`[${chalk.red("ERROR")}]${chalk.white(" - ")}${e.stack}`)
        );
      }
    }
  });
  ...
  try {
    console.log(
      chalk.white(
        `[${chalk.grey("SLASH")}]${chalk.white(" - ")}Started refreshing ${
    ...
    if (options.slash_global == true) {
      ...
      console.log(
        chalk.white(
          `[${chalk.grey("SLASH")}]${chalk.white(" - ")}Successfully reloaded ${
            global_data.length
          } application commands`
        )
      );
    ...
    console.log(
      chalk.white(
        `[${chalk.red("SLASH")}]${chalk.white(
          " - "
        )}There was an error while refreshing the application commands: ${
          e.stack
        }`
      )
    );
